...
$sql1 = mysqli_prepare($conn,"select customer_email, count(customer_email) as tot from purchases as p, flight as f, ticket as t where t.ticket_id = p.ticket_id and t.flight_num = f.flight_num and f.airline_name = ? group by customer_email order by tot desc");
mysqli_stmt_bind_param($sql,"s",$theAirline);
mysqli_stmt_execute($sql);
